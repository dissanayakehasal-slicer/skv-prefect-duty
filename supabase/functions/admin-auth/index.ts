...
serve(async (req) => {
  ...
  try {
    ...
    if (action === 'init') {
      const initPassword = typeof newPassword === 'string' && newPassword.trim().length >= 8
        ? newPassword.trim()
        : (typeof password === 'string' ? password.trim() : '');
      if (initPassword.length < 8) {
        return new Response(JSON.stringify({ success: false, error: 'Initialization password must be at least 8 characters' }), {
    ...
    if (action === 'verify') {
      ...
      if (!setting || !setting.value) {
        return new Response(JSON.stringify({ success: false, error: 'Admin password is not initialized' }), {
          status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }
